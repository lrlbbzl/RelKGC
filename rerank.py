...
def rerank_by_hop(batch_score: torch.tensor,
                    examples: List[Example],
                    entity_dict: EntityDict):
    if args.task == 'wiki5m_ind':
        assert args.neighbor_weight < 1e-6, 'Inductive setting can not use re-rank strategy'
    ...
    for idx in range(batch_score.size(0)):
        cur_ex = examples[idx]
        n_hop_indices = get_link_graph().get_nhop_entity_with_degree(cur_ex.head_id,
                                                                  entity_dict=entity_dict,
                                                                  n_hop=args.rerank_n_hop)
        for depth in range(args.rerank_n_hop):
            if depth + 1 in n_hop_indices.keys():
                delta = torch.tensor([args.neighbor_weight * (math.e ** -depth) for _ in n_hop_indices[depth + 1]]).to(batch_score.device)
                indices = torch.LongTensor(list(n_hop_indices[depth + 1])).to(batch_score.device)
