...
def construct_mask_with_ids(row_exs: List, col_entity_idx: torch.LongTensor) -> torch.tensor:
    num_row = len(row_exs)
    row_entity_idx = torch.LongTensor([entity_dict.entity_to_idx(ex.tail_id) for ex in row_exs])
    # num_row x num_col
    triplet_mask = (row_entity_idx.unsqueeze(1) != col_entity_idx.unsqueeze(0)) 
    ...
    for i in range(num_row):
        ...
        if len(neighbor_ids) <= 1:
            continue
        for j, v in enumerate(col_entity_idx):
            if entity_dict.entity_exs[v].entity_id in neighbor_ids:
                triplet_mask[i][j] = False

    return triplet_mask # Place values False will be masked which means false negative samples


def construct_mask(row_exs: List, col_exs: List = None) -> torch.tensor:
    positive_on_diagonal = col_exs is None # diagonal is positive sample
    num_row = len(row_exs)
